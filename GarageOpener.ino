...
WiFiServer server(54195); //This is the port to listen. Need to be in your router to open the door remotely. If you need only local. Put 80 instead and forget the router part
WiFiClient client;

const char* SSID = "NabZ";
const char* PASS = "9c71c5fea6";
...
void mainPage(WiFiClient& client)
{
  ...
  client.println("<hr>\r\n<br>");
  client.println("<button style=\"width:100%;font-size: 50px;\"><a href=\"/GarageOpener/activate\">Activate</a></button>");
...
void redirect(WiFiClient& client)
{
  ...
  client.println("Location: /GarageOpener");
}
...
void ProcessRequest(char* getLine)
{
  ...
  if (strstr(getLine, "GET /GarageOpener") != NULL)
  {
    if (strstr(getLine, "GET /GarageOpener/activate") != NULL)
    {
